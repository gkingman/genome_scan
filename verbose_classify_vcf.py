...
# [4] the peak key of the region of interest, ie, "chrVII:21439017:C:T"
...
for peak in vectors:
	...
	if peak != sys.argv[4]: #chrVII:21439017:C:T chrIV:12797378:A:C
		continue
	...
	matching_pops, unclear, not_matching = [], [], []
	for pop in xrange(pop_count):
		pop_scores = []
		...
		if best[-1] > 75: # Use a very conservative threshold to flag a population as matching the g0 population
			...
			unclear.append(pop_names[pop])
		else:
			not_matching.append(pop_names[pop])

	print "______________________________"
	print "Starts:"#, '\t'.join([vectors[sys.argv[4]][x][0] for x in starts])
	for x in starts:
		print x, vectors[sys.argv[4]][x][0]
	print "Stops:"#, '\t'.join([vectors[sys.argv[4]][x-1][0] for x in stops])
	for x in stops:
		print x, vectors[sys.argv[4]][x-1][0]
	...
	print peak
	print "Populations matching g0 stereotype:"
	print '\t'.join(matching_pops)
	print "Populations with unclear results:"
	print '\t'.join(unclear)
	print "Populations NOT matching g0 stereotype:"
	print '\t'.join(not_matching)
	print '________________________'
	print "Config file generated"
	for pop in pop_names:
		if pop in matching_pops:
			print pop + '\t0'
		elif pop in not_matching:
			print pop + '\t1'
		else:
			print pop
